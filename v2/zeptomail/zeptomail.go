...
// SendPasswordReset sends a branded password reset email with a clickable button.
func (m *ZeptoMailer) SendPasswordReset(ctx context.Context, email string, resetURL string, expiresIn time.Duration) error {
	minutes := int(expiresIn.Minutes())
	...
	body := map[string]any{
		...
		"htmlbody": passwordResetEmailHTML(resetURL, minutes),
	}
...
func passwordResetEmailHTML(resetURL string, minutes int) string {
	return fmt.Sprintf(`<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
    ...
    <tr>
      <td align="center">
        <table width="420" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:32px 32px 16px;">
              <p style="margin:0 0 16px;color:#71717a;font-size:14px;">You requested a password reset. Click the button below to set a new password.</p>
              <table width="100%%" cellpadding="0" cellspacing="0" style="margin:24px 0;">
                <tr>
                  <td align="center">
                    <a href="%s" style="background-color:#4f46e5;color:#ffffff;text-decoration:none;padding:12px 32px;border-radius:6px;font-weight:600;display:inline-block;">Reset Password</a>
                  </td>
                </tr>
              </table>
              <p style="margin:16px 0 0;color:#71717a;font-size:13px;">Or copy this link: <a href="%s" style="color:#4f46e5;text-decoration:none;">%s</a></p>
              <p style="margin:16px 0 0;color:#71717a;font-size:13px;">This link expires in <strong>%d minutes</strong>. If you did not request this, ignore this email.</p>
            </td>
          </tr>
          <tr>
            <td style="padding:16px 32px 32px;">
...
</html>`, resetURL, resetURL, resetURL, minutes)
}
